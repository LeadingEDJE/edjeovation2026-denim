...
engine queries against. It crawls the Abercrombie **women's** catalog and upserts
products into the `catalog_products` table.
...
```sh
# From apps/scraper. Defaults: auto-discover categories, 40 products/category.
npm run scrape
```
...
1. **Discover** women's category PLP URLs from the landing-page navigation.
...
Derivation is keyword-based, so it is accurate on denim/bottoms and intentionally
leaves attributes `NULL` where they don't apply. The full extracted payload is
kept in the `raw` JSONB column so fields can be re-derived later without
re-scraping.
