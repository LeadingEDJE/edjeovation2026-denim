...
async function inspect(
	page: Page,
	url: string,
	jsonResponses: { url: string; bytes: number }[],
) {
	console.log(`\n=== ${url} ===`);
	...
	const productAnchors = await page.evaluate(
	...
	if (jsonResponses.length) {
		...
		for (const r of jsonResponses
			...
			.slice(0, 8)) {
			console.log(
				`  ${r.bytes.toString().padStart(8)}b  ${r.url.slice(0, 100)}`,
			);
		}
	}
...
async function main() {
	const startUrl =
		process.env.URL ?? "https://www.abercrombie.com/shop/us/womens-jeans";
	...
	if (!startUrl.includes("/p/") && anchors > 0) {
		...
		if (href) {
			...
			await inspect(
				page,
				new URL(href, "https://www.abercrombie.com").toString(),
				jsonResponses,
			);
		}
	}
