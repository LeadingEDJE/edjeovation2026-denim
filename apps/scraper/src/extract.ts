...
export function deriveFit(text: string): FitPreference | null {
  ...
  if (/\bbaggy\b|\bwide\b|\bwide[- ]leg\b|\bflare\b|\bbootcut\b|\bboot[- ]cut\b|\bloose\b/.test(t)) return "wide";
  if (/\brelaxed\b|\bmom\b|\bdad\b|\b90s\b|\bbarrel\b|\bcurve love\b/.test(t)) return "relaxed";
  return null;
}

export function deriveRise(text: string): Rise | null {
  const t = text.toLowerCase();
  if (/\bultra high[- ]?rise\b|\bsuper high[- ]?rise\b/.test(t)) return "ultra-high";
...
export function deriveStretch(text: string): StretchPreference | null {
  const t = text.toLowerCase();
  if (/\bhigh[- ]?stretch\b|\bsuper stretch\b|\bextra stretch\b/.test(t)) return "high-stretch";
...
export function deriveAttributes(name: string, description: string): DerivedAttributes {
...
function extractPrice(offers?: Record<string, unknown>): { price: number | null; currency: string | null } {
  if (!offers) return { price: null, currency: null };
  if (offers.price != null) {
    return { price: toNumber(offers.price), currency: asString(offers.priceCurrency) };
  }
  const specs = offers.priceSpecification;
  const spec = (Array.isArray(specs) ? specs[0] : specs) as Record<string, unknown> | undefined;
  if (spec) return { price: toNumber(spec.price), currency: asString(spec.priceCurrency) };
