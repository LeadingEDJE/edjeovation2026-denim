...
- Associates can review customer context, assigned stylist, order-history summary, AI-assisted product suggestions, rationales, sales-floor labels, prep states, messages, notifications, session notes, recaps, and feedback.
...
- PostgreSQL stores appointments, messages, notification records, fit-profile overrides, product prep status, recaps, feedback, and the seeded catalog.
- WireMock simulates third-party customer profiles, order history, store data, stylist profiles, weekly schedules, and store inventory.
- Appointment booking assigns an eligible same-store stylist for the selected slot, creates mock confirmation/reminder records, and starts recommendation generation in the background.
...
- Stage 1: deterministic scoring ranks known catalog products using fit, stretch, waist, length, focus colors, avoid colors, and category diversity.
