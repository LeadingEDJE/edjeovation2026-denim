...
After booking, the confirmation screen says **Confirmed** and **You're booked.** It includes the assigned stylist, store, appointment time, and selected catalog. The customer can tap **Manage Appointment** to open the appointment detail. Suggestions are generated asynchronously after booking, so confirmation does not wait on the model.

<!-- SCREENSHOT: iOS splash with A&F mark and stylist wordmark -->
<!-- SCREENSHOT: iOS landing screen with Start Your Fitting -->
<!-- SCREENSHOT: iOS booking wizard showing occasion, colors, muse, catalog, outfit-to-match, store, slot, and review screens -->
...
Before the visit, the customer can message the stylist with **Message your stylist…** and **Send**, update the stylist note with **Save Note**, add outfit-to-match context, or use **Cancel Appointment**. The cancel alert confirms **Cancel this appointment?**, includes an optional **Reason optional** field, and separates **Cancel Appointment** from **Keep Appointment**.
...
<!-- SCREENSHOT: iOS appointment detail with Where, Your stylist, Outfit to match, Messages, and Save Note -->
...
### 5. Associate preps products, reviews outfit context, and messages the customer
...
The detail view also includes the customer snapshot, optional **Outfit To Match** block, current stylist, a **Reassign** selector limited to eligible same-store stylists, **Messages**, and **Notifications**. In **Messages**, associate messages are labeled **You**, customer messages are labeled **Customer**, and the associate sends a note with **Message customer…** and **Send**. **Notifications** shows mock confirmation/reminder records only; no real email or push delivery is attempted.
...
<!-- SCREENSHOT: customer snapshot, Outfit To Match, stylist, and Reassign selector -->
...
*Why it matters:* product prep, outfit context, messaging, and reassignment happen in the same workspace the associate uses for the appointment.
...
- **Regenerate or failed suggestions:** trigger **Regenerate** to refresh the shortlist. Existing suggestions stay visible while the background job runs. If generation fails, show the failed banner and retry copy.
...
- **AI unavailable fallback:** run without an AI key to show scorer-derived suggested products from the deterministic path.
  <!-- SCREENSHOT: rule-based suggested products with scorer-derived rationales -->
